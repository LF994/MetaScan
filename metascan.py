...
class CPtnMetaInfo :
  ...
  def doDirSitePackages(self, dirItm, nIdx) :
    ...
    # In metadata folder name '-' (minus) treated strictly as separator (? is is true ?)
  ...
  def tableSortByModuleName(self) :
    ...
    # simpler variant can be: lsTable.sort(key = lambda arg: arg[0].casefold())
